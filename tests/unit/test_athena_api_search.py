...
class TestAthenaSearch(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.original_db_path = collective.DB_PATH
    ...
    def tearDown(self) -> None:
        self.dao.close()
        collective.DB_PATH = self.original_db_path
        self.tmpdir.cleanup()
    ...
    def test_non_promoted_excluded(self):
        ...
        self.dao.conn.execute(
            "UPDATE collective_entries SET embedding=? WHERE id=?",
            (
                np.asarray(self.vec_a, dtype=np.float32).tobytes(),
                entry_id,
            ),
        )
        self.dao.conn.commit()
    ...
    def test_top_n_limited(self):
        for i in range(5):
            self._promote_and_embed(f"src{i}", f"mem{i}", self.vec_common)
    ...
    def test_zero_vector_returns_all_valid_entries_zero_first(self):
        nonzero_id = self._promote_and_embed("srcA", "mem1", self.vec_a)
        zero_id = self._promote_and_embed("srcB", "mem2", [0.0, 0.0])
    ...
    def test_zero_vector_ignores_null_and_malformed_embeddings(self):
        valid_id = self._promote_and_embed("srcA", "mem1", self.vec_a)
